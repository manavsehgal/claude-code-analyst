...
class MarkdownChatbot:
    ...
    def _create_claude_options(self) -> 'ClaudeCodeOptions':
        """Create Claude Code SDK options with user-configurable settings."""
        if not CLAUDE_SDK_AVAILABLE:
            return None

        # Get Claude configuration from config.yml with defaults
        claude_config = self.config.get('claude', {})

        return ClaudeCodeOptions(
            model=claude_config.get('model', 'claude-sonnet-4'),
            max_thinking_tokens=claude_config.get('max_thinking_tokens', 5000),
            max_turns=claude_config.get('max_turns', 10),
            ...
            permission_mode=claude_config.get('permission_mode', 'default')
        )
    ...
    def load_markdown_file(self, item_index: int) -> str | None:
    ...
    def _is_system_message(self, text: str) -> bool:
        """Simplified system message detection to preserve user content."""
        ...
        # Only filter obvious system/debug messages
        system_patterns = [
            r'^\(subtype=.*\)$',  # Claude Code SDK format
            r'^\(data=.*\)$',     # SDK data format
            r'duration_ms=\d+',   # SDK timing
            r'session_id=',       # SDK session info
            r'usage=\{.*\}',      # SDK usage stats
            r'^<[^>]*>$',         # XML tags on single line
            r'^\{.*\}$',          # JSON objects on single line
        ]
